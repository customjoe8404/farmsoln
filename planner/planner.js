class PlannerApp {
    constructor() {
        this.apiBaseUrl = '../php/planner-api.php';
        this.currentPlans = [];
        this.currentDate = new Date();
        this.cropData = {
            'maize': { days: 120, name: 'Maize', tasks: ['Soil Preparation', 'Planting', 'First Weeding', 'Fertilizer Application', 'Second Weeding', 'Harvesting'] },
            'wheat': { days: 90, name: 'Wheat', tasks: ['Land Preparation', 'Sowing', 'First Irrigation', 'Weeding', 'Second Irrigation', 'Harvest'] },
            'rice': { days: 150, name: 'Rice', tasks: ['Nursery Preparation', 'Transplanting', 'Water Management', 'Weeding', 'Fertilizer Application', 'Harvest'] },
            'beans': { days: 60, name: 'Beans', tasks: ['Land Preparation', 'Planting', 'Weeding', 'Harvest'] },
            'tomato': { days: 90, name: 'Tomato', tasks: ['Seedling Preparation', 'Transplanting', 'Staking', 'Pruning', 'Harvest'] },
            'potato': { days: 110, name: 'Potato', tasks: ['Land Preparation', 'Planting', 'Earthing Up', 'Harvest'] }
        };
    }
            ...
            <div class="plan-summary">
                ...
                <div class="plan-dates">
                    <div class="date-card">
                        ...
                        <div class="date-value">${this.formatDate(plan.plantingDate)}</div>
                    </div>
                    <div class="date-card">
                        <div class="date-label">Expected Harvest</div>
                        <div class="date-value">${this.formatDate(plan.harvestDate)}</div>
                    </div>
                </div>
    ...
    displayActivePlans() {
        ...
        container.innerHTML = `
            <div class="plans-grid">
                ${this.currentPlans.slice(0, 3).map(plan => `
                    <div class="plan-card">
                        ...
                        <div class="plan-details">
                            <p><strong>Area:</strong> ${plan.area} acres</p>
                            <p><strong>Planted:</strong> ${this.formatDate(plan.plantingDate)}</p>
                            <p><strong>Progress:</strong> ${this.calculatePlanProgress(plan)}%</p>
                        </div>
                        <div class="plan-actions">
                            <button onclick="plannerApp.viewPlan(${plan.id})" class="btn-outline btn-small">View</button>
                            <button onclick="plannerApp.completePlan(${plan.id})" class="btn-primary btn-small">Complete</button>
                        </div>
                    </div>
                `).join('')}
            </div>
        ...
        container.innerHTML = `
            <div class="tasks-list">
                ${upcomingTasks.map(task => `
                    <div class="task-item">
                        <div class="task-icon">${this.getTaskIcon(task.name)}</div>
                        <div class="task-content">
                            <div class="task-title">${task.name}</div>
                            <div class="task-description">For ${task.planName} • ${task.area} acres</div>
                            <div class="task-due">Due: ${this.formatDate(task.dueDate)}</div>
                        </div>
                        <button onclick="plannerApp.completeTask(${task.id})" class="btn-primary btn-small">
                            Complete
                        </button>
                    </div>
                `).join('')}
