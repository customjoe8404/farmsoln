...
try {
    ...
    $check_email = $db->prepare("SELECT id FROM system_users WHERE email = ?");
    $check_email->execute([$email]);
    if ($check_email->rowCount() > 0) {
        throw new Exception('Email already registered');
    }
    $username = strtolower(explode('@', $email)[0]);
    $check_username = $db->prepare("SELECT id FROM system_users WHERE username = ?");
    $check_username->execute([$username]);
    if ($check_username->rowCount() > 0) {
        $username = $username . '_' . rand(100, 999);
    }
    $password_hash = password_hash($password, PASSWORD_DEFAULT);
    $query = "INSERT INTO system_users (username, email, password_hash, full_name, profile_image, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, NOW(), NOW())";
    $stmt = $db->prepare($query);
    if ($stmt->execute([$username, $email, $password_hash, $full_name, $profile_image])) {
        $user_id = $db->lastInsertId();
        $get_user = $db->prepare("SELECT id, username, email, full_name, profile_image, farm_size, location, is_active, created_at, updated_at FROM system_users WHERE id = ?");
