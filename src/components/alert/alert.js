class AlertsApp {
  ...
  filterAlerts(filter) {
    ...
    switch (filter) {
      ...
      case "info":
        this.filteredAlerts = this.alerts.filter(
          (alert) => alert.severity === "info" && !alert.resolved
        );
        break;
      case "resolved":
    ...
    if (this.filteredAlerts.length === 0) {
      container.innerHTML = `
                <div class="empty-alerts">
                    ...
                    } alerts at the moment.</p>
                </div>
            `;
      return;
    }

    container.innerHTML = this.filteredAlerts
      .map(
        (alert) => `
            <div class="alert-item ${alert.severity} ${
          alert.read ? "read" : "unread"
        }">
                <div class="alert-icon">${this.getAlertIcon(
                  alert.severity
                )}</div>
                <div class="alert-content">
                    <div class="alert-header">
                        <h3 class="alert-title">${alert.title}</h3>
                        <div class="alert-time">${this.formatTime(
                          alert.timestamp
                        )}</div>
                    </div>
                    <div class="alert-message">${alert.message}</div>
                    <div class="alert-actions">
                        <button onclick="alertsApp.markAsRead(${
                          alert.id
                        ...
                        <button onclick="alertsApp.viewDetails(${
                          alert.id
  ...
  updateStats() {
    ...
    const info = this.alerts.filter(
      (a) => a.severity === "info" && !a.resolved
    ).length;
  ...
  viewDetails(alertId) {
    ...
    if (alert) {
      this.showNotification(
        `Alert details: ${alert.title} - ${alert.message}`,
        "info"
      );
    }
  }
    ...
    this.alerts = [
      {
        ...
        message:
          "Soil moisture levels in the north field have dropped to 25%. Consider irrigation to prevent plant stress.",
        severity: "warning",
      ...
      },
      {
        ...
        message:
          "High temperatures (35°C) expected tomorrow. Prepare irrigation system and consider shade for sensitive crops.",
        severity: "warning",
      ...
      },
      {
        ...
        message:
          "Aphid infestation detected in tomato crops. Monitor closely and consider organic pest control measures.",
        severity: "critical",
      ...
      },
      {
        ...
        message:
          "Scheduled maintenance for irrigation system due next week. Please inspect and service the system.",
        severity: "info",
      ...
      },
      {
        ...
        message:
          "Tomato prices have increased by 15% in the local market. Good time to consider harvesting and selling.",
        severity: "info",
    ...
    const alertTypes = [
      {
        title: "Rain Forecast",
        message:
          "Heavy rainfall (50mm) expected in 24 hours. Ensure proper drainage and protect sensitive crops.",
        severity: "warning",
      ...
      },
      {
        ...
        message:
          "Low nitrogen levels detected in soil analysis. Consider fertilizer application for optimal growth.",
        severity: "warning",
      ...
      },
      {
        ...
        message:
          "Tractor #2 requires scheduled maintenance. Please service within the next 7 days.",
        severity: "info",
