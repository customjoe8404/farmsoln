...
-- If the table already exists in an older database, add missing columns used by the application
-- Use information_schema and prepared statements so this SQL works on MySQL versions
-- that do not support `ADD COLUMN IF NOT EXISTS` in a single ALTER statement.

-- Add `profile_image` if missing
SET @exists := (
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'system_users' AND COLUMN_NAME = 'profile_image'
);
SELECT IF(@exists = 0, 'ALTER', 'SELECT') INTO @cmd;
SET @sql := IF(@exists = 0, 'ALTER TABLE system_users ADD COLUMN profile_image VARCHAR(255) DEFAULT NULL', 'SELECT 1');
...
SET @exists := (
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'system_users' AND COLUMN_NAME = 'phone'
);
SET @sql := IF(@exists = 0, 'ALTER TABLE system_users ADD COLUMN phone VARCHAR(30) DEFAULT NULL', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @exists := (
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'system_users' AND COLUMN_NAME = 'last_login'
);
SET @sql := IF(@exists = 0, 'ALTER TABLE system_users ADD COLUMN last_login DATETIME DEFAULT NULL', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
