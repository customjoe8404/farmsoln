class MarketApp {
    constructor() {
        this.apiBaseUrl = 'market_api.php';
        this.config = {
            defaultRegion: 'local',
    ...
    initializeEventListeners() {
        ...
        document.getElementById('market-region').addEventListener('change', (e) => {
    ...
    renderPriceOverview() {
        ...
        const html = `
            <div class="prices-grid">
                ${this.priceData.map(commodity => `
                    <div class="price-card ${commodity.change >= 0 ? 'price-up' : 'price-down'}">
                        <div class="commodity-header">
                            <h3>${commodity.name}</h3>
                            <span class="symbol">${commodity.commodity}</span>
                            ${commodity.source === 'mock_data' ? '<span class="mock-badge">Demo</span>' : ''}
                        </div>
                        <div class="price-main">
                            <div class="current-price">$${commodity.regional_price.toFixed(2)}</div>
                            <div class="price-change ${commodity.change >= 0 ? 'positive' : 'negative'}">
                                ${commodity.change >= 0 ? '↗' : '↘'} 
                                ${Math.abs(commodity.change).toFixed(2)} (${Math.abs(commodity.change_percent).toFixed(2)}%)
                            </div>
                        </div>
                        <div class="price-meta">
                            <span class="volume">Vol: ${commodity.volume.toLocaleString()}</span>
                            <span class="source">${commodity.source === 'alpha_vantage' ? 'Live' : 'Demo'}</span>
                        </div>
                        <div class="region-info">
                            <small>Region: ${commodity.region} (${commodity.multiplier_applied}x)</small>
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    async renderPriceTrends() {
        ...
        if (this.priceData.length > 0) {
            ...
            if (this.config.features.real_time_data) {
                ...
                document.getElementById('trend-commodity-select').addEventListener('change', (e) => {
                ...
                document.querySelectorAll('.period-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        this.loadTrendChart(
                            document.getElementById('trend-commodity-select').value,
            ...
            container.innerHTML = '<div class="no-data">No data available for trends</div>';
        }
    }
    ...
    async loadTrendChart(symbol, period = '30days') {
        ...
        if (response.success) {
            ...
            document.getElementById('price-trends').innerHTML += 
                '<div class="error-state"><p>Failed to load trends: ' + response.error + '</p></div>';
        }
    }
