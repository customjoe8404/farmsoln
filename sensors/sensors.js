class SensorApp {
    constructor() {
        this.apiBaseUrl = '../php/sensors-api.php';
    ...
    async startScan() {
        ...
        this.updateStatus('🔄', 'Scanning Field', 'Collecting sensor data from field...');
        ...
        try {
            ...
            if (data.success) {
                ...
                this.updateStatus('✅', 'Scan Complete', 'Field analysis completed successfully');
            } else {
            ...
            <div class="sensor-readings">
                ${this.sensorData.map(sensor => `
                    <div class="sensor-reading ${sensor.status}">
                        ...
                        <div class="sensor-name">${this.formatSensorName(sensor.type)}</div>
                        <div class="sensor-value">${sensor.value}</div>
                        <div class="sensor-unit">${sensor.unit}</div>
                        <div class="sensor-status-badge ${sensor.status}">${sensor.status}</div>
                    </div>
                `).join('')}
            </div>
        `;
    ...
    calculateFieldHealth() {
        ...
        this.sensorData.forEach(sensor => {
            ...
            switch(sensor.type) {
                case 'soil_moisture':
                    score = sensor.value >= 30 && sensor.value <= 70 ? 100 : 
                           sensor.value >= 20 && sensor.value <= 80 ? 70 : 40;
                    break;
                case 'temperature':
                    score = sensor.value >= 18 && sensor.value <= 32 ? 100 : 
                           sensor.value >= 15 && sensor.value <= 35 ? 80 : 60;
                    break;
                case 'ph':
                    score = sensor.value >= 6.0 && sensor.value <= 7.5 ? 100 : 
                           sensor.value >= 5.5 && sensor.value <= 8.0 ? 70 : 40;
                    break;
                case 'nitrogen':
                    score = sensor.value >= 40 && sensor.value <= 60 ? 100 : 
                           sensor.value >= 30 && sensor.value <= 70 ? 80 : 50;
                    break;
                default:
                ...
                ${recommendations.map(rec => `
                    <div class="recommendation-item ${rec.priority}">
                        ...
                        <div class="rec-content">
                            <h4>${rec.title}</h4>
                            <p>${rec.message}</p>
                            ${rec.action ? `<button class="btn-small" onclick="sensorApp.executeAction('${rec.action}')">${rec.actionText}</button>` : ''}
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    analyzeSensorData() {
        ...
        if (sensors.nitrogen && sensors.nitrogen.value < 30) {
            recommendations.push({
                ...
                icon: '🌿',
                title: 'Low Nitrogen',
                message: 'Nitrogen levels are low. Consider applying nitrogen fertilizer for better plant growth.',
                action: 'fertilize',
        ...
        if (recommendations.length === 0) {
            recommendations.push({
                ...
                message: 'All sensor readings are within optimal ranges. Continue current practices.'
            });
        }
    ...
    async executeAction(action) {
        switch(action) {
            case 'irrigate':
                this.showNotification('Irrigation scheduled for tomorrow morning', 'success');
                break;
            case 'check_drainage':
    ...
    async resetSensors() {
        this.sensorData = null;
        this.updateStatus('🔍', 'Ready to Scan', 'Click start scan to begin field analysis');
